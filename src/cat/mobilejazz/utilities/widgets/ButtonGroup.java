...
/*
 * This file modifies the original RadioButtonGroup from the android project to support not
 * only radio buttons, but any CompoundButton.
 */

package cat.mobilejazz.utilities.widgets;
...
public class ButtonGroup extends LinearLayout {
	...
	public ButtonGroup(Context context, AttributeSet attrs) {
		...
		// XML layout file
		TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.ButtonGroup);
	...
	private void setEnabledForView(int viewId, boolean enabled) {
		View checkedView = findViewById(viewId);
		if (checkedView != null && checkedView instanceof CompoundButton) {
			((CompoundButton) checkedView).setEnabled(enabled);
		}
	}
	...
	public LayoutParams generateLayoutParams(AttributeSet attrs) {
		return new ButtonGroup.LayoutParams(getContext(), attrs);
	}
	...
	// event.setClassName(ButtonGroup.class.getName());
	// }
	//
	// @Override
	...
	// info.setClassName(ButtonGroup.class.getName());
	// }
	...
	public interface OnCheckedChangeListener {
		 ...
		 */
		public void onCheckedChanged(ButtonGroup group, int checkedId);
	}
	...
	private class CheckedStateTracker implements CompoundButton.OnCheckedChangeListener {
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			...
			if (mCheckedId != -1 && mCheckedId != id) {
				setEnabledForView(mCheckedId, true);
				setCheckedStateForView(mCheckedId, false);
	...
	private class PassThroughHierarchyChangeListener implements ViewGroup.OnHierarchyChangeListener {
		...
		public void onChildViewAdded(View parent, View child) {
			if (parent == ButtonGroup.this && child instanceof CompoundButton) {
				int id = child.getId();
